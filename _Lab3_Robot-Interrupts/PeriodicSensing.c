...
volatile uint8_t bumpResultSem;                     // Sempahore for bump sensors
...
void SysTick_Handler(void){             // Handles interrupt and determines what to do with reflection sensor
    ...
    if(sysTickCounter == 0) {           // At the beginning of each cycle, begin sensor reading
        ...
        uint8_t bumpResult = Bump_Read();                   //read current states of bump sensors
        bumpResult = (~bumpResult) & ~0x12;
        bumpResultSem = bumpResult;

        switch (bumpResult) {            //There must be a better way to do this.
        case 0x00:
            Port2_Output(0x00);
            break;
        case 0x01:
